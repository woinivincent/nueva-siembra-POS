...
export interface AppSettings {
  ...
  // Precios
  cardSurcharge: number; // % recargo tarjeta
  ...
  // Sistema
  theme: 'light' | 'dark';
  lowStockAlert: number; // Umbral de alerta por defecto
}
...
export class SettingsRepository {
  ...
  getAll(): AppSettings {
    try {
      ...
      const stmt = sqlite.prepare('SELECT key, value FROM settings');
  ...
  saveAll(settings: Partial<AppSettings>): boolean {
    try {
