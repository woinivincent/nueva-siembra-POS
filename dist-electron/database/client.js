...
const dbPath = path_1.default.join(userDataPath, "pos-database.db");
if (!fs_1.default.existsSync(userDataPath)) {
    fs_1.default.mkdirSync(userDataPath, { recursive: true });
}
exports.sqlite = new better_sqlite3_1.default(dbPath);
...
function runMigrations() {
    try {
        ...
        const tableInfo = exports.sqlite
            .prepare("PRAGMA table_info(products)")
            .all();
        const hasColumn = tableInfo.some((col) => col.name === "price_card");
        if (!hasColumn) {
            console.log("📦 Ejecutando migración: agregar price_card...");
            exports.sqlite.exec(`ALTER TABLE products ADD COLUMN price_card REAL`);
            exports.sqlite.exec(`UPDATE products SET price_card = price WHERE price_card IS NULL`);
            console.log("✅ Migración completada");
        }
        // Permitir payment_method = 'mixed' en sales (pago dividido)
        const salesSqlRow = exports.sqlite
        ...
        if (!salesAllowsMixed) {
            ...
            exports.sqlite.exec("PRAGMA foreign_keys = OFF;");
            const migrateSales = exports.sqlite.transaction(() => {
                exports.sqlite.exec(`
        ...
        `);
          ...
          FROM sales;
        `);
                exports.sqlite.exec("DROP TABLE sales;");
                exports.sqlite.exec("ALTER TABLE sales_new RENAME TO sales;");
                // Re-crear índices
                exports.sqlite.exec("CREATE INDEX IF NOT EXISTS sales_customer_idx ON sales(customer_id);");
                exports.sqlite.exec("CREATE INDEX IF NOT EXISTS sales_date_idx ON sales(created_at);");
                exports.sqlite.exec("CREATE INDEX IF NOT EXISTS sales_status_idx ON sales(status);");
            });
            try {
                migrateSales();
                console.log("✅ Migración completada");
...
function seedInitialData() {
    try {
        const existingCount = exports.sqlite
            .prepare("SELECT COUNT(*) as count FROM products")
            .get();
        if (existingCount.count > 0) {
            console.log(`ℹ️ Ya existen ${existingCount.count} productos, saltando seed`);
            return;
        }
        console.log("🌱 Insertando productos de ejemplo...");
        // Ahora con price y price_card
        const insertProduct = exports.sqlite.prepare(`
    ...
    `);
        ...
        const insertMany = exports.sqlite.transaction((items) => {
        ...
        insertMany(products);
        const insertSetting = exports.sqlite.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)");
